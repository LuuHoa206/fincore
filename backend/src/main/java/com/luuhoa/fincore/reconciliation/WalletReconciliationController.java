...
public class WalletReconciliationController {
    ...
    com.luuhoa.fincore.transaction.TransactionResponse confirmAdjustment(
            @AuthenticationPrincipal Jwt jwt,
            @RequestHeader(name = "Idempotency-Key") String idempotencyKey,
            @Valid @RequestBody WalletReconciliationAdjustmentRequest request) {
        return walletReconciliationService.confirmAdjustment(UUID.fromString(jwt.getSubject()), request, idempotencyKey);
    }
}
